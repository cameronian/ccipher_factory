  ...
  module SpecDsl
    ...
    def set(field, value)
      if @fields.keys.include?(field)
        ...
        raise CcipherFactory::SpecDslError, "Field '#{field}' is not defined inside the spec"
      end
    end # set
      ...
      if @fields.keys.include?(field)
        if not_empty?(@fields[field])
          if @fields[field].is_a?(TSpec)
            ...
            raise CcipherFactory::SpecDslError, "Field '#{field}' is not an expected TSpec object. Please use set() for non tspec field"
          end
        else
          raise CcipherFactory::SpecDslError, "Field '#{field}' is not defined inside the spec"
        end
        ...
        if not_empty?(@ofields[field])
          if @ofields[field].is_a?(TSpec)
            ...
            raise CcipherFactory::SpecDslError, "Optional field '#{field}' is not an expected TSpec object. Please use set() for non tspec field"
          end
        else
          raise CcipherFactory::SpecDslError, "Optional field '#{field}' is not defined inside the spec"
        end
      else
        raise CcipherFactory::SpecDslError, "Field '#{field}' is not defined inside the spec"
      end
    ...
    def required_but_still_empty_fields
      res = []
      @fields.each do |k,v|
        res << k if v[:value].nil? #is_empty?(v[:value])
      end
      res.sort
    end
    ...
    def to_bin(opts = { }, &block)
      raise CcipherFactory::SpecDslError, "Block is required" if not block
    ...
    def to_array(opts = { }, &block)
      ...
      raise CcipherFactory::SpecDslError, "Block is required" if not block
      raise CcipherFactory::SpecDslError, "Spec is not completed! The following keys are still vacant : [#{required_but_still_empty_fields.join(", ")}]" if not is_completed?
      ...
      @field_seq.each do |f|
        ...
        if @fields.keys.include?(f)
          ...
          raise CcipherFactory::SpecDslError, "Mandatory field '#{f}' cannot be empty" if is_empty?(val)
          ...
          raise CcipherFactory::SpecDslError, "Field '#{f}' is not defined everywhere!"
        end
    ...
    def load_from_array(asn1, &block)
      ...
      if not_empty?(asn1)
        ...
        raise CcipherFactory::SpecDslError, "ASN.1 is empty"
      end 
    end
    ...
    def create_field(id, ftype, defVal = "", opts = { }, &block)
      ...
      raise CcipherFactory::SpecDslError, "Given id '#{id}' already defined" if @fields.keys.include?(id)
    ...
    def create_optional_field(id, ftype, defVal = "", opts = { }, &block)
      ...
      raise CcipherFactory::SpecDslError, "Given id '#{id}' already defined" if @ofields.keys.include?(id)
  ...
  class Tspec
    ...
    def initialize(path, opts = { })
      ...
      if File.exist?(path)
        ...
        raise CcipherFactory::SpecDslError, "Given Tspec '#{path}' not found"
      end
    end # initialize
  ...
  class SpecDslBuilder
    ...
    def self.from_array(asn1, opts = { }, &block)
      ...
      if not_empty?(asn1)
        ...
        if not_empty?(opts[:verify_tspec])
          raise CcipherFactory::SpecDslError, "Expected '#{opts[:verify_tspec]}' but loaded '#{const}'" if opts[:verify_tspec] != const
        end
        ...
        if not_empty?(const)
          ...
          raise CcipherFactory::SpecDslError, "Cannot get constant from value '#{val}'"
        end
      else
        raise CcipherFactory::SpecDslError, "Given data to load from ASN.1 is empty"
      end 
    ...
    def self.from_bin(bin, opts = { }, &block)
      ...
      if not bin.nil? 
        ...
        raise CcipherFactory::SpecDslError, "Given binary to load into ASN.1 is empty"
      end 
    ...
    def self.version_int_to_string(verInt)
      ...
      when 0x0100
        ...
        raise CcipherFactory::SpecDslError, "Unknown version integer '#{verInt}'"
      end
    end
    ...
    def self.version_string_to_int(verStr)
      ...
      when "1.0"
        ...
        raise CcipherFactory::SpecDslError, "Unknown version string '#{verStr}'"
      end
    end
    ...
    def self.load_tspec(val,version = "1.0")
      ...
      if File.exist?(specPath)
        ...
        raise CcipherFactory::SpecDslError, "Tspec '#{specPath}' not found"
      end
...
if $0 == __FILE__
  inst = CcipherFactory::SpecDslBuilder.instance(:kdf_scrypt_envp)
  p inst
