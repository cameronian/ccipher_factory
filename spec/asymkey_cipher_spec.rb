...
RSpec.describe CcipherFactory::AsymKeyCipher do
  ...
  it 'encrypts and decrypts for ECC recipients' do
    ...
    enc.encrypt_init #({ recipient_public: recp.public_key })
    ...
    dec.decrypt_init #(recp)
    dec.decrypt_update_meta(ts)
    dec.decrypt_update_cipher(out.string)
    dec.decrypt_final
